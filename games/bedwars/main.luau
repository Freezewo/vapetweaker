...
local GrimReaperSpeedEnabled = false
...
local function getSpeed()
	...
	local baseSpeed = (20 + (knockbackBoost > tick() and knockbackSpeed or 0)) * (multi + 1)
	...
	if GrimReaperSpeedEnabled and lplr.Character and lplr.Character:GetAttribute('GrimReaperChannel') then
		baseSpeed = baseSpeed + 20
	end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/MaxlaserTech/CatV6/'..readfile('vapetweaker/profiles/commit.txt')..'/'..select(1, path:gsub('vapetweaker/', '')), true)
		end)
...
local suc, err = pcall(function()
	Packages = httpService:JSONDecode(downloadFile('vapetweaker/profiles/packages.json'))
end)
if not suc then
	warn('[VapeTweaker] Failed to load packages.json:', err)
	warn('[VapeTweaker] Some features may not work correctly')
end
local debug = debug
...
run(function()
	if not canDebug then
		...
		vape.Libraries.cheatenginelib = loadstring(downloadFile('vapetweaker/games/bedwars/engine.luau'))(vape, vapeEvents, entitylib, store, bedwars)
	end
end)
		...
		if remoteName == remotes.AttackEntity then
			return {
				...
				SendToServer = function(_, attackTable, ...)
					...
					if Reach and Reach.Enabled or HitBoxes and HitBoxes.Enabled or getgenv().Backtrack and getgenv().Backtrack.Enabled or getgenv().FakeLag and getgenv().FakeLag.Enabled then
	...
	calculatePath = function(target, blockpos, findmag, angle)
        ...
        for _ = 1, 10000 do
            ...
            for _, side in sides do
                ...
                local plrangle = math.acos((gameCamera.CFrame.LookVector * Vector3.new(1, 0, 1)):Dot(((((block.Position - localPosition))) * Vector3.new(1, 0, 1)).Unit))
...
run(function() 
	...
	local function findAim(localcframe, predicted, fps, started, offset)
		...
		return localcframe:Lerp(CFrame.lookAt(localcframe.p, predicted + Vector3.new(
			(rng:NextNumber() - 0.5) * Shake.Value * fps,
			offset + ((rng:NextNumber() - 0.5) * Shake.Value * fps),
			(rng:NextNumber() - 0.5) * Shake.Value * fps
		)), speed * fps), speed
	end
	...
	BowAssist = vape.Categories.Combat:CreateModule({
		Name = 'Bow Assist',
		Tooltip = 'Smoothly aims to best predicted target position with projectile item',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local TP
	local ProgressBar
	local DamageAnimation
	local CameraAnimation
	local ProgressBarColor
	local rayCheck = RaycastParams.new()
	rayCheck.RespectCanCollide = true
	local up, down, old = 0, 0, nil
	local progressFrame, oldcamupdate, camcontrol
	local onground, lastonground, groundtime = true, true, 0
	...
	Fly = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				...
				if not flyAllowed and not hadBalloons and DamageAnimation.Enabled and shared.damageanim then
					shared.damageanim()
					pcall(function()
				...
				if ProgressBar.Enabled and not flyAllowed and not hadBalloons and progressFrame then
					progressFrame.Visible = true
					progressFrame.Frame:TweenSize(UDim2.new(1, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0, true)
				end
				...
				Fly:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive and isnetworkowner(entitylib.character.RootPart) then
						...
						if ProgressBar.Enabled and progressFrame then
							progressFrame.Visible = not flyAllowed
							if not flyAllowed then
								local newray = workspace:Raycast(root.Position + Vector3.new(0, -3, 0), Vector3.new(0, -1000, 0), rayCheck)
								onground = newray and true or false
								if lastonground ~= onground then
									if not onground then
										groundtime = tick() + 2.6
										progressFrame.Frame:TweenSize(UDim2.new(0, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, groundtime - tick(), true)
									else
										progressFrame.Frame:TweenSize(UDim2.new(1, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0, true)
									end
								end
								progressFrame.TextLabel.Text = math.max(onground and 2.5 or math.floor((groundtime - tick()) * 10) / 10, 0)..'s'
	...
	DamageAnimation = Fly:CreateToggle({
		Name = 'Damage Animation',
		Function = function(callback)
			if callback then
				task.spawn(function()
					repeat task.wait(0.1) until gameCamera
					for _, v in pairs(getconnections(gameCamera:GetPropertyChangedSignal('CameraType'))) do
						if v.Function then
							camcontrol = debug.getupvalue(v.Function, 1)
						end
					end
					if not camcontrol then return end
					...
					shared.damageanim = function()
						...
						if CameraAnimation.Enabled then
							num.Value = 1000
				...
				if camcontrol and oldcamupdate then
					camcontrol.Update = oldcamupdate
				end
			end
		end
	})
	CameraAnimation = Fly:CreateToggle({
	...
	ProgressBar = Fly:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				progressFrame.Position = UDim2.new(0.5, 0, 1, -200)
				progressFrame.Size = UDim2.new(0.2, 0, 0, 20)
				progressFrame.BackgroundTransparency = 0.5
				progressFrame.BorderSizePixel = 0
				progressFrame.BackgroundColor3 = Color3.new(0, 0, 0)
				progressFrame.Visible = Fly.Enabled
				progressFrame.Parent = vape.gui
				local progressFrame2 = progressFrame:Clone()
				progressFrame2.AnchorPoint = Vector2.new(0, 0)
				progressFrame2.Position = UDim2.new(0, 0, 0, 0)
				progressFrame2.Size = UDim2.new(1, 0, 0, 20)
				progressFrame2.BackgroundTransparency = 0
				progressFrame2.Visible = true
				progressFrame2.Name = 'Frame'
				progressFrame2.Parent = progressFrame
				local progressText = Instance.new('TextLabel')
				progressText.Text = '2s'
				...
				local function updateColor()
					if ProgressBarColor then
						progressFrame.BackgroundColor3 = Color3.fromHSV(ProgressBarColor.Hue, ProgressBarColor.Sat, ProgressBarColor.Value)
						progressFrame.Frame.BackgroundColor3 = Color3.fromHSV(ProgressBarColor.Hue, ProgressBarColor.Sat, ProgressBarColor.Value)
					end
				end
				updateColor()
				Fly:Clean(vape.Categories.Friends.ColorUpdate.Event:Connect(updateColor))
			else
				if progressFrame then
	...
	ProgressBarColor = Fly:CreateColorSlider({
		Name = 'Bar Color',
		Function = function()
			if progressFrame then
				progressFrame.BackgroundColor3 = Color3.fromHSV(ProgressBarColor.Hue, ProgressBarColor.Sat, ProgressBarColor.Value)
						...
						if #plrs > 0 then
							...
							for i, v in plrs do
								...
								if actualRoot then
									...
									if FastHits.Enabled and (FireRate.Value <= 0 or (tick() - lastShot) >= FireRate.Value) and responded and i <= 1 and not entitylib.Wallcheck(entitylib.character.RootPart.Position, targetpos, {gameCamera, lplr.Character, v})  then
			...
			if callback then
				...
				if position then
					if Movement.Value == 'All' then
						if not getgenv().MouseTPs.Kits(position) and not getgenv().MouseTPs.Items(position) then
							notif('MouseTP', 'Couldn\'t find an item or a kit to teleport with', 5)
						end
					elseif not getgenv().MouseTPs[Movement.Value](position) then
						...
						if ent then
							...
							for _, data in getProjectiles() do
								...
								if (FireDelays[item.itemType] or 0) < tick() then
									...
									local meta = bedwars.ProjectileMeta[projectile]
									local projSpeed, gravity = meta.launchVelocity, meta.gravitationalAcceleration or 196.2
									local calc = prediction.SolveTrajectory(pos, (projSpeed * (1 - lplr:GetNetworkPing())), gravity, ent.RootPart.Position, ent.RootPart.Velocity * Vector3.new(1, 0, 1), workspace.Gravity, ent.HipHeight, ent.Jumping and 42.6 or nil, rayCheck)
									if calc then
										targetinfo.Targets[ent] = tick() + 1
			...
			if callback then
				Speed:Clean(runService.PreSimulation:Connect(function(dt)
					...
					if entitylib.isAlive and not Fly.Enabled and not LongJump.Enabled and isnetworkowner(entitylib.character.RootPart) then
						entitylib.character.Humanoid.WalkSpeed = Mode.Value == 'CFrame' and 20 or Value.Value
...
run(function()
	...
	DamageBoost = vape.Categories.Blatant:CreateModule({
		Name = 'Damage Boost',
		Tooltip = 'Boosts your speed while being damaged',
		Function = function(callback) end
	})
	
	bedwars.ZapNetworking.EntityDamageEventZap.On(function(entityInstance, damage, damageType, fromPosition, fromEntity)
		if entitylib.isAlive and entityInstance == lplr.Character and damage > 4 and fromEntity and bedwars.matchState ~= 0 and DamageBoost.Enabled and not Fly.Enabled then
...
run(function()
	...
	ZephyrExploit = vape.Categories.Blatant:CreateModule({
		Name = 'Zephyr Exploit',
		Tooltip = 'Resets Zephyr ability cooldown on kill (Wind Walker kit)',
		Function = function(callback)
			if callback then
				ZephyrExploit:Clean(vapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if entitylib.isAlive and bedwars.matchState ~= 0 and store.equippedKit == 'wind_walker' then
						local killer = playersService:GetPlayerFromCharacter(deathTable.fromEntity)
						local killed = playersService:GetPlayerFromCharacter(deathTable.entityInstance)
						
						if killer and killed and killer == lplr and killed ~= lplr then
							pcall(function()
								bedwars.CooldownController:setOnCooldown('wind_walker_jump', 0, false)
							end)
						end
					end
				end))
...
run(function()
	...
	NoPlacementCPS = vape.Categories.Blatant:CreateModule({
		Name = 'No Placement CPS',
		Tooltip = 'Removes block placement CPS limit',
		Function = function(callback)
			if callback then
				oldCPS = bedwars.SharedConstants.BLOCK_PLACE_CPS
				bedwars.SharedConstants.BLOCK_PLACE_CPS = math.huge
			else
				if oldCPS then
					bedwars.SharedConstants.BLOCK_PLACE_CPS = oldCPS
				end
			end
		end
	})
...
run(function()
	local GrimReaperSpeedModule
	local Range
	
	GrimReaperSpeedModule = vape.Categories.Blatant:CreateModule({
		Name = 'Grim Reaper Speed',
		Tooltip = 'Gives +20 speed when GrimReaper channel is active and auto collects souls',
		Function = function(callback)
			GrimReaperSpeedEnabled = callback
			...
			if callback then
				task.spawn(function()
					repeat
						...
						if entitylib.isAlive and store.equippedKit == 'grim_reaper' then
							local localPosition = entitylib.character.RootPart.Position
							
							for pos, soul in bedwars.GrimReaperController.soulsByPosition do
								if not GrimReaperSpeedModule.Enabled then break end
								
								if soul and soul:IsA('Model') then
									local soulPart = soul.PrimaryPart or soul:FindFirstChildWhichIsA('BasePart')
									if soulPart then
										local distance = (soulPart.Position - localPosition).Magnitude
										
										if distance <= Range.Value then
											local secret = soul:GetAttribute('GrimReaperSoulSecret')
											if secret then
												pcall(function()
					...
					until not GrimReaperSpeedModule.Enabled
				end)
			end
		end
	})
	
	Range = GrimReaperSpeedModule:CreateSlider({
		Name = 'Soul Range',
		Min = 10,
		Max = 120,
		Default = 30
			...
			if callback then 
				BulletTracers:Clean(workspace.ChildAdded:Connect(function(projectile)
					task.delay(0, function()
						...
						local origin = entitylib.character.RootPart.Position + Vector3.yAxis
						local size, velocity = 1, Vector3.zero
						...
						if DrawingToggle.Enabled then 
							...
							repeat
								if projectile.PrimaryPart then
									size, velocity = (origin - projectile.PrimaryPart.Position).Magnitude, projectile.PrimaryPart.Velocity
									drawingobjs[obj] = {origin, origin + (velocity.Unit * (size * 2)), tick()}
								end
								task.wait()
							...
							obj.Size = Vector3.new(0.05, 0.05, size * 2)
							obj.CFrame = CFrame.lookAt(origin + (velocity.Unit * size), origin + (velocity.Unit * (size * 2)))
							...
							pcall(function()
								velocity = projectile.PrimaryPart.Velocity
								repeat
									if projectile.PrimaryPart then
										size, velocity = (origin - projectile.PrimaryPart.Position).Magnitude, velocity
										obj.Size = Vector3.new(0.05, 0.05, size * 2)
										obj.CFrame = CFrame.lookAt(origin + (velocity.Unit * size), origin + (velocity.Unit * (size * 2)))
									end
									task.wait()
							...
							if Fade.Enabled then 
								local tween = tweenService:Create(obj, TweenInfo.new(1), {
									Transparency = 0.5
								})
					...
					BulletTracers:Clean(runService.RenderStepped:Connect(function()
						for obj, data in drawingobjs do 
							...
							if vis and vis2 then
								...
								if Fade.Enabled then 
									obj.Transparency = Color.Opacity * (1 - math.clamp((tick() - data[3]) / 2, 0, 1))
								end
							else
								obj.Visible = false
							end
	...
	local function firePearl(pos, spot, item)
		if Check.Enabled then
			for _, v in store.selfProjectiles do
				if v.Parent and v.Name == 'telepearl' then
					return
				end
			end
		end
		...
		if calc then
			...
			local res = projectileRemote:InvokeServer(item.tool, 'telepearl', 'telepearl', pos, pos, dir, httpService:GenerateGUID(true), {drawDurationSeconds = 1, shotId = httpService:GenerateGUID(false)}, workspace:GetServerTimeNow() - 0.045)
			task.spawn(function()
				repeat 
					task.wait()
				until not projectile or not projectile.Parent
	...
	local function shootFunc(ignore)
		if not inputService.MouseEnabled or ignore then
			...
			if proj then
				...
				if ent then
					calc = prediction.SolveTrajectory(selfpos, projSpeed, meta.gravitationalAcceleration or 196.2, Vector3.new(ent.RootPart.Velocity.X, 0, ent.RootPart.Velocity.Z), workspace.Gravity, ent.HipHeight, nil, RaycastParams.new(), nil, lplr:GetNetworkPing())
				end
                    ...
                    if Bedfinder.Enabled and not getBed() then
                        ...
                        if PlaceMode.Value == 'Smart' then
							...
							for _, offset in {Vector3.new(0, -2, 0), Vector3.new(0, 1, 0)} do
								local placement = workspace:Raycast(selfpos + offset, entitylib.character.RootPart.CFrame.LookVector * 4, rayCheck)
...
run(function()
	...
	vape.Components.HotbarList = function(optionsettings, children, api)
		...
		textbuttonicon.Image = getcustomasset('vapetweaker/assets/new/add.png')
...
run(function()
	...
	TrapESP = vape.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				repeat task.wait() until store.matchState ~= 0 or not TrapESP.Enabled
			...
			if callback then
				...
				repeat
					...
					for _, v in workspace:QueryDescendants('#Summoner_SummonCircle') do
						...
						if math.floor(pivot.Position.X) == math.floor(entitylib.character.RootPart.Position.X) and math.floor(pivot.Position.Z) == math.floor(entitylib.character.RootPart.Position.Z) then
							abilityActive = true
							break
						end
					end
					...
					if plr then
						...
						if PerfectAbility.Enabled and targetDistance <= AbilityDistance.Value then
							if (currentTime - lastAbilityTime) >= abilityCooldown then
								...
								if chargeTime >= 0.5 then
									...
									replicatedStorage['events-@easy-games/game-core:shared/game-core-networking@getEvents.Events'].useAbility:FireServer('summoner_finish_charging')
						...
						if (currentTime - lastAttackTime) >= attackCooldown then
							if abilityActive and not SwingDuringAbility.Enabled then
								task.wait(0.05)
								continue
							end
							...
							if ShowAnimation.Enabled then
								task.spawn(pcall, function()
									...
									if (gameCamera.CFrame.Position - entitylib.character.RootPart.Position).Magnitude < 1 then
										for _, v in clawModel:GetDescendants() do
									...
									task.deay(0.75, clawModel.Destroy, clawModel)
								end)
							end
							bedwars.Client:Get(remotes.SummonerClawAttack):SendToServer({
								position = localPosition,
...
run(function()
    ...
    AutoEmber = vape.Categories.Utility:CreateModule({
        Name = 'Auto Ember',
        Function = function(call)
            if call then
                repeat
...
run(function()
    ...
    AutoUma = vape.Categories.Utility:CreateModule({
        Name = 'Auto Uma',
...
run(function()
    ...
    AutoAdetunde = vape.Categories.Utility:CreateModule({
        Name = 'Auto Adetunde',
...
run(function()
    ...
    AutoPyro = vape.Categories.Utility:CreateModule({
        Name = 'Auto Pyro',
...
run(function()
    ...
    AutoNoelle = vape.Categories.Utility:CreateModule({
        Name = 'Auto Noelle',
...
run(function()
    ...
    AutoMarina = vape.Categories.Utility:CreateModule({
        Name = 'Auto Marina',
...
run(function()
    ...
    AutoBuilder = vape.Categories.Utility:CreateModule({
        Name = 'Auto Builder',
        Function = function(call)
            if call then
...
run(function()
    ...
    AutoNyx = vape.Categories.Utility:CreateModule({
        Name = 'Auto Nyx',
...
run(function()
    ...
    AutoSheep = vape.Categories.Utility:CreateModule({
...
run(function()
    ...
    AutoMelody = vape.Categories.Utility:CreateModule({
        Name = 'Auto Melody',
...
run(function()
    ...
    AutoRamil = vape.Categories.Utility:CreateModule({
        Name = 'Auto Ramil',
...
run(function() 
	...
	AutoCaitlyn = vape.Categories.Utility:CreateModule({
        Name = 'Auto Caitlyn',
        Function = function(callback)
                ...
                if entitylib.isAlive and damageTable.fromEntity == lplr.Character and (entitylib.character.RootPart.Position - damageTable.entityInstance.HumanoidRootPart.Position).Magnitude <= Range.Value then                                
                    ...
                    if not state.Kit.activeContract then
                        for _, v in (state.Kit.availableContracts or {}) do
                            if v.target and v.target.Name == damageTable.entityInstance.Name and (Mode == 'On Hit' or damageTable.entityInstance.Humanoid.Health < MinHP.Value) then
...
run(function()
    ...
    AutoZeno = vape.Categories.Utility:CreateModule({
        Name = 'Auto Zeno',
        Function = function(call)
            if call then
                repeat
...
run(function()
    ...
    AutoKaliyah = vape.Categories.Utility:CreateModule({
        Name = 'Auto Kaliyah',
...
run(function()
    ...
    AutoElder = vape.Categories.Utility:CreateModule({
        Name = 'Auto Elder',
...
run(function()
    ...
    AutoTaliyah = vape.Categories.Utility:CreateModule({
        Name = 'Auto Taliyah',
...
run(function()
    ...
    AutoStar = vape.Categories.Utility:CreateModule({
...
run(function()
    ...
    AutoBee = vape.Categories.Utility:CreateModule({
        Name = 'Auto Beekeeper',
...
run(function() -- Sigrid
    ...
    SigridExploit = vape.Categories.Utility:CreateModule({
